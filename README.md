...
### Benefit
해당 프로젝트는 Hand Recognition 진행했지만 더 나아가 사람 몸을 탐지할 수 있는 Body Recognition을 통해서 일어날 수 있는 사고에 대한 
데이터들을 수집 데이터 학습을 통해 예측 및 임계값을 부여하여 주의를 감지할 수 있게 적용한다면 위험 요소를 사전에 방지하며 보다 안전한 사회를 만들 수 있을 것
...
- **<개선점>**
  ...
  - scissors가 다른 action에 비해서 예측확률이 현저히 떨어짐 why???
    ...
    ==> 예측확률을 추가적으로 확인함으로써 모델 성능에 의존하면 위험성이 존재할 수 있음을 확인

    ==> Data split 후 추가적으로 타겟에 대한 비율 확인 필요성 확인
  - 예측확률이 낮은데 예측한 행동은 정확한가???
    * 예측확률이 현저히 낮을 때는 임계값을 설정(= 특정 행동에 대한 이상치 탐지용으로 적용 가능, 모델이 정확하단 판단하에 진행해야함)
...
- **<어려웠던 점>**
  ...
  - **코사인 유사도 적용(= 구현)**
    ...
    - 구현하기 위한 코딩역량 부족
      - [알고리즘 학습(진행중)](https://github.com/kimmy-git/Algorithm_Test) 
      
- **<느낀점>**
  - scissors는 두 가지 형태로 보여질 수 있는데(= V or 가위 형태) 현재 V형태로 학습
    - 둘 다 똑같은 뜻인데 가위형태는 scissors로 안보고 rock or paper로 판단
