...
결국 사람을 대신할 기술이 개발되어지는 것인데 이러한 기술적인 부분에서 사람은 가장 큰 장애물 중 하나일 수 있다.
도로에서 보다 안전한 자율 주행을 위해서는 주변의 운전자, 자전거 타는 사람 및 보행자 등이 다음에 무슨 행동을 할 것 인가에 대해 예측할 수 있어야 한다.

### 문제 정의
특정 행동에 대한 데이터 수집 및 label 부여하고 Mediapipe API를 사용한 행동 예측

### 목표
Mediapipe Hand API를 사용하여 수집한 데이터에 대한 예측 모델 구현 및 성능 파악

### Benefit
해당 프로젝트는 Hand detection만 진행했지만 더 나아가 사람 몸을 탐지할 수 있는 Body detection을 통해서 일어날 수 있는 사고에 대한 데이터들을 수집하고 행동에 대한 임계값을 부여하여 주의를 감지할 수 있게 적용한다면 위험 요소를 사전에 방지하며 보다 안전한 사회를 만들 수 있을 것
...
## Results
* Model 성능 100%
  * Mediapipe 성능 확인 및 모델 구현을 하는데 목적을 둠
    * 선택된 모델(=RF, LSTM) 직접 테스트를 통해 대체적으로 다 좋은 예측 성능을 보임
...
* 느낀점
  ...
  - 영상 인식 할 수 있는 라이브러리에 대한 공부가 필요함
  ...
  - scissors는 두 가지 형태로 보여질 수 있는데(= V or 가위 형태) 현재 V형태로 학습
    ...
    - 내가 주먹이라고 해서 다른 사람이 똑같이 주먹이라고 보지 않을 것임
      - 사람의 사고가 다 똑같지 않기 때문에 label에 문제점이 많을 것이라 느낌
      - 결국 해당 행동에 대해서 어떻게 정의를 할 것인가에 대한 고민이 많을 것 같다.   

* 개선점
  - 모델 성능은 100%인데, scissors가 다른 action에 비해서 예측확률이 현저히 떨어짐
    - data split 문제(= 재진행 필요)
    - paper가 test_data로 들어가 있음, 각 클래스마다 data split 진행 후 concate
